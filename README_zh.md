...
模拟数据集：500 用户，1,000 视频，15,000 次交互，**正样本率 14.1%**。

> **关于 AUC ≈ 0.5 的说明**：模拟数据上属于预期行为——watch_ratio 标签与特征之间没有真实相关性（随机生成）。在真实 KuaiRec 数据上，DeepFM 通常能达到 AUC 0.72–0.78。

### 召回阶段

| 模型 | Recall@10 | NDCG@10 | Hit@10 | Recall@50 | 训练轮数 |
|------|-----------|---------|--------|-----------|---------|
| Two-Tower (MeanPool, in-batch) | **0.1693** | 0.1258 | 0.2270 | 0.2520 | 30 |
| Two-Tower (MeanPool, random neg) | 0.1005 | — | — | 0.1730 | 8* |
| Two-Tower + **SASRec** | — | — | — | — | — |

*消融快速训练（8 epochs）。完整 SASRec：`python src/training/train_retrieval.py --seq_model sasrec`
...
|------|-----------|----------|------------|-----------|
| **MMoE**（4 专家） | **0.5272** | **0.5123** | 0.3497¹ | 0.4484¹ |

> ¹ 模拟数据上 Like AUC < 0.5：合成数据中 `like` 标签（点击型二值标签）与特征之间的真实相关性极低。真实 KuaiRec 数据上 MMoE Like AUC 通常可达 0.68–0.72。

### 消融实验（各 8 epochs）

| 召回变体 | Recall@10 | Recall@50 | 结论 |
|---------|-----------|-----------|------|
| In-batch 负采样 | 0.0597 | 0.1391 | 高效但训练信号弱 |
| **Random 负采样** | **0.1005** | **0.1730** | +68% Recall@10（8ep 对比） |
| 无序列特征 | 0.0416 | 0.0862 | −30%，序列特征至关重要 |

| 排序变体 | AUC | GAUC | 结论 |
|---------|-----|------|------|
| **DeepFM（完整）** | **0.5231** | 0.5086 | FM 交叉项有效 |
| DeepFM（无 FM 项） | 0.5173 | 0.4910 | 去掉 FM 项 −0.006 AUC |
| MLP 基线 | 0.5080 | 0.5215 | 无特征交叉 |
| DIN | 0.4976 | 0.4840 | 真实数据效果更佳 |

完整分析见 [`experiments/results/ablation_report.md`](experiments/results/ablation_report.md)。
