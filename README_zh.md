...
合成 KuaiRec 数据集：500 用户，~991 视频，15,000 次交互，**正样本率 37.9%**。
数据生成器使用**个性化物品采样**（用户-类别亲和力 → 相关 watch_ratio 标签）。

### 召回阶段

| 模型 | Recall@10 | NDCG@10 | Hit@10 | Recall@50 |
|------|-----------|---------|--------|-----------|
| Two-Tower (MeanPool) | 0.157 | 0.121 | 0.260 | 0.269 |
| Two-Tower + **SASRec** | **0.177** | **0.146** | **0.278** | **0.328** |

SASRec 通过因果自注意力机制使 Recall@10 提升 **+12.7%**。
...
|------|-----------|----------|------------|-----------|
| **MMoE**（4 专家） | 0.5009 | **0.5117** | 0.5125 | **0.5195** |
...
| MeanPool（完整训练） | 0.0029 | 0.0286 | 消融基线 |
...
| DIN | 0.5440 | 0.5122 | 注意力机制提升 GAUC |
